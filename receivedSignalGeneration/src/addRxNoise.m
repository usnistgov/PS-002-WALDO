function s = addRxNoise(s, snr)
%ADDRXNOISE AWGN noise 
%
%   R = ADDRXNOISE(S, SNR) add white Gaussian noise to the signal s. s
%   power is assumed to be 1 (0 dBW). SNR is specidied in dB.
%   
...
snrLin = 10^(snr/10);
n = sqrt(1/(2*snrLin)) * ( randn(size(s)) + 1i*randn(size(s)) );
s = s + n;
